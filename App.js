...
export default function App() {
  ...
  const shareTournament = async (t) => {
    try {
      const htmlContent = generateTournamentReportHTML('FULL_REPORT');
      console.log("Generated HTML Content (first 500 chars):", htmlContent.substring(0, 500));
      ...
      if (Platform.OS === 'android') {
        ...
        const newFilePath = FileSystem.cacheDirectory + 'DeepPitch_Report_Share_' + Date.now() + '.pdf';
        await FileSystem.writeAsStringAsync(newFilePath, fileContent, { encoding: FileSystem.EncodingType.Base64 });
        console.log("PDF re-written to new path:", newFilePath);
        
        shareUri = await FileSystem.getContentUriAsync(newFilePath);
        console.log("Converted to Android Content URI for sharing:", shareUri);
      }
  ...
  function generateMatches(teams, mode, groupSize = 4, quals = 2) {
    ...
    const teamIds = shuffledTeams.map(t => t.id || t.name);
  ...
  const exportAsCSV = async () => {
    ...
    try {
      ...
      activeT.matches.forEach(m => {
        csv += `${m.id},${m.group ? 'Group '+m.group : (m.round ? 'Round '+m.round : 'League')},${getTN(m.home)},${getTN(m.away)},${m.hScore},${m.aScore},${m.status},${m.winner || ''}\n`;
      });
  ...
  const generateTournamentReportHTML = (type) => {
    if (!activeT) return '';
    const isFullReport = type === 'FULL_REPORT';
    const escapedName = escapeHtml(activeT.name);
    const escapedType = escapeHtml(activeT.type);

    const renderStandings = () => {
      if (activeT.type === 'GROUPS') {
        const groups = [...new Set(activeT.matches.filter(m => m.group).map(m => m.group))];
        return groups.map(g => `
          <h3 style="color:#00FF66; margin-top:15px">Group ${escapeHtml(g)}</h3>
          <table>
            <tr><th style="text-align:left">TEAM</th><th>P</th><th>W</th><th>D</th><th>L</th><th>GD</th><th>PTS</th></tr>
            ${getStats({...activeT, matches: activeT.matches.filter(m => m.group === g)}).map(([id, s]) => `
              <tr><td class="team-name">${escapeHtml(getTN(id))}</td><td>${s.p}</td><td>${s.w}</td><td>${s.d}</td><td>${s.l}</td><td>${s.gd}</td><td>${s.pts}</td></tr>
            `).join('')}
          </table>
        `).join('');
      }
      ...
      return `
        <table>
          ...
          ${stats.map(([id, s]) => `
            <tr>
              <td class="team-name">${escapeHtml(getTN(id))}</td>
      ...
      return `
        <table>
          ...
          ${activeT.matches.map(m => `
            <tr>
              <td class="team-name">${escapeHtml(getTN(m.home))} vs ${escapeHtml(getTN(m.away))}</td>
              <td style="color:#A0A0A0; font-size:10px">${m.group ? 'Group '+escapeHtml(m.group) : (m.round ? 'Round '+m.round : 'League')}</td>
              <td>${m.done ? `<b>${m.hScore} - ${m.aScore}</b>` : (m.status === 'LIVE' ? '<span style="color:#00FF66">LIVE</span>' : 'PENDING')}</td>
            </tr>
          `).join('')}
        </table>
      `;
      ...
      <html>
        <head>
          <style>
            ...
            h1 { color: #00FF66; text-align: center; margin-bottom: 5px; }
            h2 { color: #FFD700; border-bottom: 2px solid #222; padding-bottom: 5px; margin-top: 30px; }
            .meta { color: #A0A0A0; text-align: center; margin-bottom: 20px; font-size: 14px; }
            table { width: 100%; border-collapse: collapse; margin-top: 10px; }
            th, td { border-bottom: 1px solid #222; padding: 10px; text-align: center; font-size: 12px; }
            th { color: #00FF66; text-transform: uppercase; }
        ...
        <body>
          <h1>${escapedName}</h1>
          <div class="meta">${escapedType} Tournament • ${activeT.teams.length} Teams</div>
  ...
  const exportAsPDF = async (type) => {
    try {
      const htmlContent = generateTournamentReportHTML(type);
      console.log("Generated HTML Content (first 500 chars):", htmlContent.substring(0, 500));
      ...
      if (Platform.OS === 'android') {
        const fileContent = await FileSystem.readAsStringAsync(printTempUri, { encoding: FileSystem.EncodingType.Base64 });
        const newFilePath = FileSystem.cacheDirectory + 'DeepPitch_Report_' + Date.now() + '.pdf';
        await FileSystem.writeAsStringAsync(newFilePath, fileContent, { encoding: FileSystem.EncodingType.Base64 });
        console.log("PDF re-written to new path:", newFilePath);
      ...
      await Sharing.shareAsync(shareUri, { mimeType: 'application/pdf', dialogTitle: `Share Report` });
    } catch (e) {
